...
async function createAdminUser() {
  console.log("🔐 Obscura Labs Admin Account Setup (Neon)")
  console.log("==========================================\n")
      ...
      if (user.role === "admin") {
        ...
        if (update.toLowerCase() === "y" || update.toLowerCase() === "yes") {
          ...
          await sql(
            `
            UPDATE users 
            SET password_hash = $1, name = $2, updated_at = CURRENT_TIMESTAMP 
            WHERE email = $3
          `,
        ...
        if (promote.toLowerCase() === "y" || promote.toLowerCase() === "yes") {
          ...
          await sql(
            `
            UPDATE users 
            SET role = $1, password_hash = $2, name = $3, updated_at = CURRENT_TIMESTAMP 
            WHERE email = $4
          `,
